...
from renga_flow.config.dataset_library_ref import (
...
from renga_flow_ui.dataset_form import loads_for_training, strip_display_name_from_toml
...
def build_training_export_zip(
    ...
    *,
    ...
    for _field, entry in iter_dataset_path_strings(config):
        ...
        if is_library_dataset_ref(entry):
            ...
            add_dataset_toml(
                ...
                source_path=None,
                map_keys=[entry, canonical_dataset_ref(entry), f"renga-flow-dataset:{did}"],
            )
            continue
    ...
    readme = (
        "Renga Flow training export\n"
        "========================\n\n"
        f"1. Extract this ZIP to a folder.\n"
        f"2. Run: python -m renga_flow.main --config {main_name}\n\n"
