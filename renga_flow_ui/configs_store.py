...
write_config_text = library_db.write_config_text
...
def list_configs_summary() -> list[dict[str, Any]]:
    return library_db.list_configs_summary()


def search_configs_page(q: str, *, page: int, page_size: int) -> dict[str, Any]:
    return library_db.search_configs(q, page=page, page_size=page_size)
...
def _resolve_dataset_value(value: str, job_staging: Path) -> str:
    ...
    if library_db.is_library_dataset_ref(value):
        ...
        content = library_db.read_dataset_text(did)
...
def materialize_staging(
    ...
    *,
    ...
    if "dataset" in config and isinstance(config["dataset"], str):
        ds_path = _resolve_dataset_value(config["dataset"], job_staging)
...
def import_example(src: Path, config_id: str | None = None) -> str:
    cid = library_db._safe_id(config_id or src.stem)
    library_db.write_config_text(cid, src.read_text(encoding="utf-8"))
    return cid


def write_config_temp_for_validate(config_id: str) -> Path:
