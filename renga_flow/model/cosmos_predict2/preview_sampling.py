...
from renga_flow.model.cosmos_predict2.layers import NoopOffloader
from renga_flow.model.cosmos_predict2.text import compute_text_embeddings, tokenize
from renga_flow.model.cosmos_predict2.vae import vae_decode
from renga_flow.utils.common import round_to_nearest_multiple
    ...
    with torch.inference_mode(), _preview_autocast(pipeline):
        for i in range(num_steps):
            ...
            if (i + 1) % 5 == 0 or i + 1 == num_steps:
                print(f"renga_flow: preview Euler {i + 1}/{num_steps}", flush=True)
...
def generate_preview_image(
    ...
    print("renga_flow: encoding preview prompt...", flush=True)
    ...
    print(f"renga_flow: Euler sampling ({num_steps} steps, cfg={guidance_scale})...", flush=True)
    ...
    print("renga_flow: VAE decode for preview...", flush=True)
    image = decode_latents_to_pil(pipeline, latents)
    print("renga_flow: preview image ready.", flush=True)
    return image
