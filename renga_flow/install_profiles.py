"""Optional dependency profiles for ``renga init`` / Maintenance UI."""
...
def renga_init_command(profiles: list[str]) -> str:
    normalized = normalize_profiles(profiles)
    if normalized == ["base"]:
        return "renga init"
    return "renga init " + " ".join(normalized)
        ...
        rows.append(
            {
                ...
                "command": renga_init_command([key]),
            }
        )
    return rows
