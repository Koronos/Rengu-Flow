...
from renga_flow.config.local_config import ensure_local_config_loaded, repo_root
from renga_flow.cli.project_venv import ensure_ui_dependencies, reexec_cli
...
def _ensure_node() -> None:
    ...
    if not which("npm"):
        raise SystemExit(
            "renga: npm not found. Install Node.js or nvm, then run: cd ui/web && npm ci && npm run build"
        )
...
def _build_web(root: Path, *, force: bool = False) -> None:
    ...
    if not dist.is_file():
        raise SystemExit(f"renga: build finished but {dist} is missing")
...
def _free_port(port: int, *, patterns: tuple[str, ...]) -> None:
    ...
    for line in result.stdout.splitlines():
        ...
        if any(p in cmd for p in patterns):
            ...
            raise SystemExit(f"renga: port {port} in use by PID {pid}: {cmd.strip()}")
        break
...
def _wait_health(
    ...
    *,
    ...
    if token:
        curl_base.extend(["-H", f"X-Renga-Flow-Token: {token}"])
    attempt = 0
...
def _serve(host: str, port: int, *, reload: bool = False) -> None:
    ...
    if reload:
        from renga_flow_ui import settings as ui_settings
...
def run(args: argparse.Namespace) -> None:
    ...
    if cmd == "start":
        ...
        host = cfg.ui.host
        port = cfg.ui.port
        _free_port(port, patterns=("renga", "uvicorn", "renga-flow-ui"))
    ...
    if cmd == "dev":
        ...
        _free_port(
            api_port,
            patterns=("renga", "uvicorn", "renga-flow-ui", "renga_flow_ui", "multiprocessing"),
        )
        ...
        if not _wait_health(health, cfg.ui.token, timeout=health_timeout, label="API"):
            ...
            api_log_fh.close()
            raise SystemExit(
                "renga: API did not become healthy for dev mode. "
                f"See {api_log} or: renga ui serve --reload"
            )
        ...
        npm_env["RENGA_FLOW_UI_PORT"] = str(api_port)
        npm_env["RENGA_FLOW_UI_DEV_PORT"] = str(dev_port)
        npm_cmd = ["npm", "run", "dev"]
        if not args.no_open:
            npm_cmd.extend(["--", "--open"])
        try:
