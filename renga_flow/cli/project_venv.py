...
from renga_flow.cli.uv_cmd import run_uv_sync_or_exit, run_uv_venv
...
def venv_renga(root: Path | None = None) -> Path:
    return venv_dir(root) / "bin" / "renga"
...
def ensure_project_venv(root: Path | None = None) -> Path:
    ...
    run_uv_venv(root)
    if not py.is_file():
        raise SystemExit("renga: uv venv finished but .venv/bin/python is missing")
    return py
...
def reexec_cli(argv: list[str] | None = None) -> None:
    """Run the same CLI from ``.venv/bin/renga`` when the project venv exists."""
    ...
    if Path(sys.executable).resolve() == py.resolve():
        return
    if renga.is_file():
        os.execv(str(renga), [str(renga), *argv])
    os.execv(str(py), [str(py), "-m", "renga_flow.cli", *argv])
