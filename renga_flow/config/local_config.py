"""Load repo-root ``renga.local.toml`` for CLI, UI, and training launcher defaults."""
...
LOCAL_CONFIG_FILENAME = "renga.local.toml"
LOCAL_CONFIG_EXAMPLE = "renga.local.toml.example"
...
def parse_local_config_dict(data: dict[str, Any], *, root: Path) -> LocalConfig:
    ...
    ui = UiConfig(
        ...
        data_dir=str(ui_raw.get("data_dir", ".renga-flow-ui")),
...
def load_local_config(path: Path | None = None, *, root: Path | None = None) -> LocalConfig | None:
    """Parse ``renga.local.toml`` if present. Returns None when the file is missing."""
    global _loaded
    r = root or repo_root()
...
def apply_local_config_to_environ(cfg: LocalConfig | None = None) -> None:
    ...
    os.environ["RENGA_FLOW_UI_HOST"] = c.ui.host
    os.environ["RENGA_FLOW_UI_PORT"] = str(c.ui.port)
    os.environ["RENGA_FLOW_UI_DATA"] = str(c.ui_data_dir())
    if c.ui.token:
        os.environ.setdefault("RENGA_FLOW_UI_TOKEN", c.ui.token)
    os.environ.setdefault("RENGAFLOW_MAINTENANCE", "1" if c.maintenance.enabled else "0")
    os.environ.setdefault(
        "RENGAFLOW_MAINTENANCE_ALLOW_PIP",
        "1" if c.maintenance.allow_pip else "0",
    )


def init_local_config_file(*, root: Path | None = None, force: bool = False) -> Path:
    """Copy example to ``renga.local.toml`` when missing (or when ``force``)."""
    r = root or repo_root()
