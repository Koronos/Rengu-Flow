"""Parse and format ``renga-flow-dataset:`` refs in training configs.

Refs may include an optional human-readable suffix after the numeric id::

    renga-flow-dataset:3
    renga-flow-dataset:3:artista 1
...
def canonical_dataset_ref(value: str) -> str:
    """Normalize to ``renga-flow-dataset:<id>`` (strip display suffix)."""
    if not is_library_dataset_ref(value):
        return value.strip()
...
def collect_script_dataset_library_ref_issues(config: dict[str, Any]) -> list[str]:
    """Errors when training config uses UI library refs outside the Renga Flow UI."""
    issues: list[str] = []
    ...
    for field, entry in iter_dataset_path_strings(config):
        ...
        try:
            ...
            issues.append(
                ...
                "or run training from the Renga Flow UI."
            )
            continue
        ...
        export_hint = (
            f"Export dataset #{did} from the Renga Flow UI (Datasets → open dataset → Export) "
            "and set that .toml path in your config."
        )
        issues.append(
            f"{field_label} uses UI-only library reference {entry.strip()!r}. "
            "Refs like `renga-flow-dataset:` are resolved only when a job is started from the UI. "
            f"For `python -m renga_flow.main` or other script runs, {export_hint}"
        )
