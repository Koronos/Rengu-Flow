...
| **kprodigy** | Memory-efficient parameter-free Prodigy (D-adaptation); train at `lr=1.0` and it discovers the LR itself, at Adafusion-style memory (`koptim`) | [K-Optimizers](https://github.com/Koronos/K-Optimizers), [KProdigy docs](https://github.com/Koronos/K-Optimizers/blob/main/docs/kprodigy.md) |
| **autofusion** | Parameter-free LR on Adafusion via a Mechanic tuner (not Prodigy); train at `lr=1.0`, the tuner finds the scale then freezes to plain Adafusion (`koptim`) | [K-Optimizers](https://github.com/Koronos/K-Optimizers), [Autofusion docs](https://github.com/Koronos/K-Optimizers/blob/main/docs/autofusion.md) |
| **liofusion** | Lion sign-momentum on Adafusion's quantized-momentum backend; absolute-minimum state (no 2nd moment, ~0.5 B/param at 4bit) (`koptim`) | [K-Optimizers](https://github.com/Koronos/K-Optimizers), [Liofusion docs](https://github.com/Koronos/K-Optimizers/blob/main/docs/liofusion.md) |
...
`adafusion`, `muon`, `adamuon`, `kprodigy`, `autofusion`, and `liofusion` come from the git-backed [`koptim`](https://github.com/Koronos/K-Optimizers) package and are installed on demand via the **koptim** install profile when you select one of these types.
...
| **autofusion** | `lr` → `1.0` (parameter-free — keep it at `1.0`), `adafusion_betas` → `[0.0, 0.999]`, `momentum_dtype` → `"bfloat16"`, `bf16_method` → `"stochastic_rounding"` |
| **liofusion** | `lr` → `2e-4`, `betas` → `[0.95, 0.98]` (classic Lion; loss↔generalization dial), `weight_decay` → `0.0`, `momentum_dtype` → `"bfloat16"`, `cautious` → `true`, `bf16_method` → `"stochastic_rounding"` |
...
| **d0**, **d_coef** | **prodigy**, **kprodigy** | D-adaptation initial estimate and scale; tune `d_coef` (not `lr`) to force larger/smaller adaptive LR |
...
| parameter-free `lr` | **prodigy**, **kprodigy**, **autofusion** | Keep `lr = 1.0`; these discover the effective LR themselves (Prodigy D-adaptation; Autofusion's Mechanic tuner) |
| **eps** | **adafusion**, **adamuon** | Two floats `[eps_factored, eps_clip]` (default `[1e-30, 1e-3]`). (**kprodigy** uses a single scalar `eps`, default `1e-8`) |
...
| **momentum_dtype** | **adafusion**, **muon**, **adamuon**, **kprodigy**, **autofusion**, **liofusion** | Momentum storage: `"float32"`, `"bfloat16"`, `"int8"`, or `"4bit"`. bf16 keeps state at ~2 B/param (int8 ~1 B, 4bit ~0.5 B). For Adafusion, set `betas` to `[0.0, …]` for a true no-momentum (lowest-memory) run |
| **cautious** | **adafusion**, **adamuon**, **kprodigy**, **liofusion** | Cautious update masking; helps with momentum (on by default for AdaMuon/Liofusion; off by default for KProdigy). For Adafusion, set `false` when `betas[0] = 0.0` (no momentum), where it's a no-op |
...
| **bf16_method** | **adafusion**, **muon**, **adamuon**, **kprodigy**, **autofusion**, **liofusion** | bf16-correct weight update: `"stochastic_rounding"` (no extra buffer), `"kahan"`, or `"none"` |
...
| **adafusion_betas** | **autofusion** | Inner Adafusion momentum betas (default `[0.0, 0.999]`; `beta1=0` gives the bit-exact freeze) |
| **betas** (Lion dial) | **liofusion** | `β2` trades loss vs generalization: `[0.95, 0.98]` (classic Lion) lowest loss; `[0.9, 0.99]` balanced |

For the full koptim parameter set (e.g. `foreach` batching, `momentum_4bit_block`, Muon's `nesterov`, Autofusion's `lr_freeze`/`scale_cap`), see the [koptim docs](https://github.com/Koronos/K-Optimizers/tree/main/docs). Any key under `[optimizer]` is forwarded to the constructor, so unlisted kwargs work too.
