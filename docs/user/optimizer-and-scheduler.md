...
| **adakaon** | Conv-aware factored optimizer; AdamW-quality at 1–2 B/param with bf16-correct updates. Triton `fused` step enabled by default in Rengu (`kaon`) | [K-Optimizers](https://github.com/Koronos/K-Optimizers), [Adakaon docs](https://github.com/Koronos/K-Optimizers/blob/main/docs/adakaon.md) |
...
| **adakaon** | `lr` → `1e-4`, `auto_lr` → `false` (quarantined; `true` is rejected), `betas` → `[0.9, 0.999]`, `eps` → `[1e-30, 1e-3]`, `weight_decay` → `0.0`, `clip_threshold` → `1.0`, `momentum_dtype` → `"bfloat16"`, `cautious` → `true`, `fused` → `true`, `bf16_method` → `"stochastic_rounding"` |
...
| **adapnm** | `lr` → `1e-3`, `betas` → `[0.8, 0.999]` (`beta1` is the loss↔gap dial), `beta0` → `0.5` (PN coefficient; `0` = plain Adam), `weight_decay` → `0.0`, `cautious` → `true`, `momentum_dtype` → `"bfloat16"`, `fused` → `true`, `bf16_method` → `"stochastic_rounding"` |
...
| **nekaon** | `lr` → `1e-4`, `k` → `1.5` (lookahead steps; loss↔gap dial), `betas` → `[0.5, 0.999]` (`beta1` regime knob, must be `> 0`), `weight_decay` → `0.1`, `cautious` → `true`, `momentum_dtype` → `"4bit"` (~0.56 B/param), `fused` → `true`, `bf16_method` → `"stochastic_rounding"` |
...
| **auto_lr** | Kaon optimizers | Quarantined in Kaon 0.7.6+: keep `false`. `true` raises before training so an experimental controller cannot damage a run |
...
| **momentum_dtype** | **adakaon**, **adamuon**, **kprodigy**, **lion**, **adapnm**, **adabelief**, **adamp**, **adopt**, **schedulefree** | Momentum storage: `"float32"`, `"bfloat16"`, `"int8"`, or `"4bit"`. bf16 keeps state at ~2 B/param (int8 ~1 B, 4bit ~0.5 B). For Adakaon, set `betas` to `[0.0, …]` for a true no-momentum (lowest-memory) run |
