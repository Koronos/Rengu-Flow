...
| **`max_checkpoints_to_keep`** | Maximum number of `global_step*` checkpoint folders to keep in the run directory. Oldest are deleted after each new checkpoint. | Positive integer. `latest` always points at the newest checkpoint. The `wsd` scheduler's `predecay` fork is **not** a `global_step*` folder, so it is never counted or pruned by this limit (see [optimizer-and-scheduler](optimizer-and-scheduler.md#extending-a-wsd-run-the-predecay-fork)). | Omitted (keep all checkpoints). |
