...
Generated by `scripts/poc_cpu_ram_optimizations.py` on synthetic SDXL-shaped tensors (512 latents 4×64×64, micro-batch 4). Re-run locally:

```bash
python scripts/poc_cpu_ram_optimizations.py --json tmp/poc_cpu_ram_results.json
```

## Default policy (rengu-flow)
...
- **`cache_dedup_text_embeddings`** — opt-in; see [training-techniques.md](training-techniques.md).

## Not implemented (backlog)

1. **Safetensors pack per bucket** — alternative to stacked `.bin` if needed later — [BACKLOG P2-2](../BACKLOG.md).
