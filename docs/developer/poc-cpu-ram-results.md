...
- **`Cache.get_many`** — groups reads by shard (`renga_flow/utils/cache.py`).
- **`SizeBucketDataset.get_items_batch`** + **`ConcatenatedBatchedDataset`** fast path when a micro-batch hits one bucket.
- **Cache v2** — `renga_flow/utils/cache_v2.py`, wired via `cache_format` in `_map_and_cache` / `DatasetManager`.
