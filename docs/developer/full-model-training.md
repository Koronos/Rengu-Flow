...
The decision is in `renga_flow/main.py`: `is_adapter = bool(config.get("adapter"))`. The **Saver** (`renga_flow/utils/saver.py`) branches on `is_adapter` in `save_model()`: adapter path calls `save_adapter(name)`, full-model path calls `save_full_model(name)`.
...
2. **model.save_model(save_dir, diffusers_sd)**  
   - Implemented per model (e.g. SDXL in `renga_flow/model/sdxl.py`).  
...
The method is optional on the model contract (default no-op in `renga_flow/model/base.py`).
