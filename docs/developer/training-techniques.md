...
| Key | Maps to | Values |
|-----|---------|--------|
| `compile_mode` | `torch.compile(mode=...)` | unset → `"default"`; `"reduce-overhead"` (CUDA-graph based, lowest per-step launch overhead — best for fixed-shape steps, costs a little extra VRAM for graph pools); `"max-autotune"`; `"max-autotune-no-cudagraphs"` |
| `compile_dynamic` | `torch.compile(dynamic=True)` | `true` when input shapes vary between steps (avoids per-shape recompiles) |

The first steps pay a one-time Inductor/CUDA-graph **warmup** (graphs build), then steady-state steps are faster — worthwhile when the run is long enough to amortize the slow early steps. Short smokes mix warmup into the mean and are not representative; judge steady-state iter time after warmup. Measured figures live in the [Cosmos LoKR user doc](../user/training-cosmos-predict2-lora-lokr-finetune.md) (steady steps ~0.51 s vs ~0.68–0.70 s without compile). `reduce-overhead` can further reduce step time over the default mode on fixed-shape steps; benchmark it per setup.
