...
- **kaon aliases** (lazy import, git-backed `kaon` profile): `adakaon`, `adamuon`, `kprodigy`, `lion`, `adapnm`, `adabelief`, `adamp`, `adopt`, `schedulefree`, `lookahead`, `sam`, `msam`, `nekaon` — also in `OPTIMIZER_ALIASES` (module `"kaon"`). Selecting one routes to the `kaon` install profile (`install/manager.py` derives the trigger set from these aliases). AutoLR remains quarantined; do not add trainer-side probe logic.
