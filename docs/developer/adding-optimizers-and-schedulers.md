...
- **kaon aliases** (lazy import, git-backed `kaon` profile): `adakaon`, `muon`, `adamuon`, `kprodigy`, `autokaon`, `lion`, `adapnm` — also in `OPTIMIZER_ALIASES` (module `"kaon"`). Selecting one routes to the `kaon` install profile (`install/manager.py` derives the trigger set from these aliases).
