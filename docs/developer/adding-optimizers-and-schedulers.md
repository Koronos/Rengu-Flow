...
- **koptim aliases** (lazy import, git-backed `koptim` profile): `adafusion`, `muon`, `adamuon`, `kprodigy`, `autofusion`, `liofusion` — also in `OPTIMIZER_ALIASES` (module `"koptim"`). Selecting one routes to the `koptim` install profile (`install/manager.py` derives the trigger set from these aliases).
