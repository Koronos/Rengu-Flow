...
- **kaon aliases** (lazy import, git-backed `kaon` profile): `adakaon`, `adamuon`, `kprodigy`, `autokaon`, `lion`, `adapnm`, `adabelief`, `adamp`, `adopt`, `schedulefree`, `lookahead`, `sam` — also in `OPTIMIZER_ALIASES` (module `"kaon"`). Selecting one routes to the `kaon` install profile (`install/manager.py` derives the trigger set from these aliases).
