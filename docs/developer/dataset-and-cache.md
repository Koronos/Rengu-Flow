...
`subsample_ratio` (fraction) and `max_images` (absolute count) are two **mutually exclusive** limiters on how many **base images** a folder serves per epoch. The cap is **per folder**, applied **once** over the folder's base images and shared across all of its resolution/AR size buckets — *not* per bucket. (Resolution buckets + augmentation branches expand each selected base image normally, so the actual rows served = selected-base-images × augmentation × the active resolution.) Both go through the same rotation machinery, governed by `subsample_shuffle` (renamed from the retired `static_sampling`, inverted polarity). Neither is baked into the cached iteration order — the full pool stays cached so the served window can change per epoch (the latents cache was always full regardless, so this adds no caching cost):

- `DirectoryDataset.folder_subsampler()` lazily builds one `FolderSubsampler` over the union of base keys (`image_spec_base`) across the folder's size buckets. `cap = effective_sample_cap(base_count, max_images, subsample_ratio)` — `max_images`, else `max(1, int(base_count*ratio))` when `ratio < 1`, else `None` (uncapped). `selected(epoch)` returns `cap` base keys (rotating window, or frozen when static; `cap > base_count` repeats up to the cap so small folders fill their quota).
- Each `SizeBucketDataset` filters `iteration_order` to the rows whose base image the folder selected this epoch (`_served_rows`); `_effective_len`/`_pool_index`/`__len__` derive from that. Uncapped, it serves the whole pool — reshuffled per epoch (`RandomCursor`) when `subsample_shuffle` is on, static order otherwise. The pure helper `rotation_window_index(pos, epoch, n, cap, static)` does the per-epoch window math (now over base images inside `FolderSubsampler`).
...
- **`rengu_flow.utils.cache.Cache`** — Single disk format: `manifest.json` (fingerprint, tensor specs), `tensors/{key}.bin` (stacked payloads; float tensors stored as **bf16**), `meta.db` (per-index JSON for non-tensor fields and optional null tensors). Opened via **`rengu_flow.utils.cache.open_disk_cache`**, which rejects legacy v1 caches (a `metadata.db` file) with an actionable error.
- **`rengu_flow.data.cache_utils._map_and_cache`** — Maps over a HuggingFace `datasets.Dataset` with a `map_fn(example, rank)`, persists results via `open_disk_cache`. Fingerprint from dataset `_fingerprint` + optional `new_fingerprint_args`. If `map_fn is None`, loads existing cache only (used after worker cache run). CPU preprocessing is parallelized on a `ThreadPoolExecutor` (`cache_num_proc` threads, default `min(8, cpu_count)`) while the GPU encode (VAE, text encoders) stays serialized in the main process — each worker thread marshals its preprocessed tensor to the main process via a per-rank `Pipe` handoff. Threads, not an `mp.Pool`: the GPU encode is the bottleneck (the pool sat mostly idle), and threads share the in-process queue without any fork-after-CUDA hazard. `num_proc <= 1` (Windows, or explicit) runs the map on the calling thread.
...
- **`rengu_flow.data.manager.DatasetManager`** — Holds model (VAE, text encoders), registers datasets, and runs **`cache()`**: obtains a `(worker, queue)` pair from `backend.make_cache_worker(_cache_fn, args)` — for `"accelerate"` (single-GPU) this is a `threading.Thread` + in-process `queue.Queue`; for `"deepspeed"` (multi-GPU) it is an `mp.Process` + `mp.Manager().Queue()` broadcast to all ranks. The worker runs `_cache_fn` (metadata → latents → text embeddings) while the main process handles GPU encode work from the queue. After cache, VAE/TE can be unloaded; then all ranks load datasets from cache (`cache_metadata(trust_cache=True)`, `cache_latents(None, trust_cache=True)`, `cache_text_embeddings(None, i)`).
- **Do not fork GPU/CUDA workers**: do not replace the `ThreadPoolExecutor` with an `mp.Pool`. `mp.Pool.fork` after CUDA init deadlocks, and a thread-local queue cannot cross a process fork — both were live deadlocks that the thread design fixed.
...
| Main flow | `rengu_flow.main`: loads dataset config; if real data: `Dataset`, `DatasetManager`, `cache()`, `--cache_only` exit; after engine init (`select_backend` / `build_engine`): `train_data.post_init(...)` or synthetic, then `PipelineDataLoader`; CLI: `--cache_only`, `--regenerate_cache`, `--trust_cache`, `--dump_dataset` (see `rengu_flow.data.dump_dataset`) |
