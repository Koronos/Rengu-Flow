...
| [`rengu_flow/cli/train_launcher.py`](../../rengu_flow/cli/train_launcher.py) | DeepSpeed argv + `[training.env]` merge |
...
| [`rengu_flow/main.py`](../../rengu_flow/main.py) | `parse_args` / `run_prepared` — still used by DeepSpeed `-m rengu_flow.main` |
...
DeepSpeed subprocesses should continue to use `-m rengu_flow.main` so the CLI does not need to be on `PATH` inside workers.
