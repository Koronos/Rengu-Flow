...
The SDXL pipeline in `rengu_flow/model/sdxl.py` delegates to these modules: it does not implement LoRA/LoKr itself. It calls `networks.lora_sdxl` or `networks.lokr_sdxl` based on `config['adapter']['type']`.
