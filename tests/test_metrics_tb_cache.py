"""Tests for the TensorBoard scalar read cache (now in rengu_track.reader; UI uses a shim)."""

from __future__ import annotations

import pytest

from rengu_track import reader

pytestmark = pytest.mark.no_ui_db


def test_read_scalars_caches_until_mtime_changes(tmp_path, monkeypatch) -> None:
    run = tmp_path / "run"
    run.mkdir()
    reader.invalidate_scalars_cache()
    loads: list[int] = []

    def fake_load(_run_dir):
        loads.append(1)
        return {"train/loss": [{"step": 1, "value": 0.5, "wall_time": 0.0}]}

    mtime = {"v": 1.0}

    monkeypatch.setattr(reader, "_load_scalars", fake_load)
    monkeypatch.setattr(reader, "_latest_event_mtime", lambda _rd: mtime["v"])

    a = reader.read_scalars(run)
    b = reader.read_scalars(run)
    assert a == b
    assert len(loads) == 1

    mtime["v"] = 2.0
    c = reader.read_scalars(run)
    assert c == a
    assert len(loads) == 2


def test_invalidate_scalars_cache(tmp_path, monkeypatch) -> None:
    run = tmp_path / "run"
    run.mkdir()
    reader.invalidate_scalars_cache()
    loads: list[int] = []

    monkeypatch.setattr(
        reader,
        "_load_scalars",
        lambda _rd: loads.append(1) or {"train/x": []},
    )
    monkeypatch.setattr(reader, "_latest_event_mtime", lambda _rd: 1.0)

    reader.read_scalars(run)
    reader.invalidate_scalars_cache(run)
    reader.read_scalars(run)
    assert len(loads) == 2
