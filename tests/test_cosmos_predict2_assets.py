...
def test_qwen3_assets_exist():
    root = resources.files("renga_flow.model.cosmos_predict2").joinpath("assets", "qwen3_06b")
    assert Path(str(root / "config.json")).is_file()
    assert Path(str(root / "tokenizer.json")).is_file()


def test_t5_assets_exist():
    root = resources.files("renga_flow.model.cosmos_predict2").joinpath("assets", "t5_old")
    assert Path(str(root / "config.json")).is_file()
