...
def test_ref_with_display_suffix() -> None:
    ref = "renga-flow-dataset:3:artista 1"
    ...
    assert canonical_dataset_ref(ref) == "renga-flow-dataset:3"


def test_ref_without_suffix() -> None:
    ref = "renga-flow-dataset:12"
    assert library_dataset_id_from_ref(ref) == 12
    assert library_dataset_label_from_ref(ref) is None


def test_format_ref() -> None:
    assert dataset_library_ref(5, "My set") == "renga-flow-dataset:5:My set"
    assert dataset_library_ref(5) == "renga-flow-dataset:5"


def test_invalid_ref() -> None:
    with pytest.raises(ValueError):
        library_dataset_id_from_ref("renga-flow-dataset:abc")
