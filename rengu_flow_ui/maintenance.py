...
from rengu_flow_ui import optional_deps
...
def maintenance_allow_pip() -> bool:
    for key in ("RENGUFLOW_MAINTENANCE_ALLOW_PIP", "RENGAFLOW_MAINTENANCE_ALLOW_PIP"):
        val = os.environ.get(key, "").strip().lower()
        if val in ("1", "true", "yes", "on"):
            return True
    return False
...
def submodule_update() -> dict[str, Any]:
    ...
    if not _gitmodules_entries():
        return {
            "ok": True,
            "returncode": 0,
            "stdout": "No .gitmodules in this repository (rengu-flow vendors Cosmos code in-tree).\n",
            "stderr": "",
            "command": [],
...
def deps_install(profile: str, *, execute: bool, confirm: bool) -> dict[str, Any]:
    ...
    if not execute:
        return {
            ...
            "profile": profile,
            "command": command_str,
            "message": "Copy and run at repo root, or enable RENGUFLOW_MAINTENANCE_ALLOW_PIP=1 and pass execute=true.",
        }
    if not maintenance_allow_pip():
        raise ValueError(
            "Server-side install is disabled. Set RENGUFLOW_MAINTENANCE_ALLOW_PIP=1 to allow execute=true."
        )
    if not confirm:
