...
"""Krea 2 transformer (single-stream MMDiT), vendored from diffusers `main`
(``models/transformers/transformer_krea2.py``, ``Krea2Transformer2DModel``).

Vendored because the class only exists in unreleased diffusers (>= 0.39.dev) and the
pinned release is older. Adapted for training here: the attention-processor indirection,
PEFT mixin and LoRA-scale plumbing are removed (rengu attaches adapters through its own
networks seam), attention runs through ``F.scaled_dot_product_attention``, and the
gradient-checkpointing branch is dropped (the trainer checkpoints at the pipeline-layer
level). Module and parameter names are unchanged, so the official checkpoints load
verbatim via ``from_pretrained`` and full-model exports stay diffusers-compatible.
...
class Krea2RMSNorm(nn.Module):
    """RMSNorm with a zero-centered scale: the effective multiplier is `1 + weight`, matching the
    Krea 2 checkpoint format. Activations are upcast so the normalization runs in float32."""
...
class Krea2Attention(nn.Module):
    """Self-attention with grouped-query projections, q/k RMSNorm, rotary embeddings and a
    sigmoid output gate."""

    def __init__(
        self, hidden_size: int, num_heads: int, num_kv_heads: int | None = None, eps: float = 1e-5
    ) -> None:
        super().__init__()
...
class Krea2TextFusionBlock(nn.Module):
    """Pre-norm transformer block (no rotary embeddings, no time modulation) used by the text
    fusion stage."""
...
class Krea2TextFusion(nn.Module):
    ...
    Two `layerwise_blocks` attend across the `num_text_layers` axis independently for every token,
    a linear `projector` collapses that axis, and two `refiner_blocks` attend across the token
    sequence.
    """
...
class Krea2RotaryPosEmbed(nn.Module):
    # modified from https://github.com/black-forest-labs/flux (FluxPosEmbed lineage)
...
class Krea2Transformer2DModel(ModelMixin, ConfigMixin):
    r"""
    The single-stream MMDiT flow-matching backbone used by Krea 2.

    Text conditioning enters as a stack of hidden states tapped from several layers of a
    multimodal text encoder. A small text-fusion transformer collapses the layer axis and refines
    the token sequence; the result is concatenated with the patchified image latents into a single
    `[text, image]` sequence processed by the transformer blocks. The timestep conditions every
    block through one shared modulation vector plus per-block learned tables.
    """
    ...
    def build_attention_masks(
        encoder_attention_mask: torch.Tensor | None, image_seq_len: int
    ) -> tuple[torch.Tensor | None, torch.Tensor | None]:
        """Key-padding masks of shape (B, 1, 1, L): padded text tokens are excluded as attention
        ...
        return text_attention_mask, attention_mask
    ...
    def forward(
        ...
        Args:
            hidden_states: `(batch_size, image_seq_len, in_channels)` packed (patchified) noisy
                image latents.
            encoder_hidden_states: `(batch_size, text_seq_len, num_text_layers, text_hidden_dim)`
                stack of tapped text-encoder hidden states per token.
            timestep: `(batch_size,)` flow-matching time in `[0, 1]` (1 is pure noise, 0 is clean
                data).
            position_ids: `(text_seq_len + image_seq_len, 3)` `(t, h, w)` rotary coordinates for
                the combined sequence. Text rows are all-zero; image rows hold the latent-grid
                coordinates.
            encoder_attention_mask: `(batch_size, text_seq_len)` boolean mask marking valid text
                tokens, or `None` when every text token is valid.
        ...
        text_attention_mask, attention_mask = self.build_attention_masks(
            encoder_attention_mask, image_seq_len
        )
...
def prepare_position_ids(text_seq_len: int, grid_height: int, grid_width: int, device) -> torch.Tensor:
    """Build the `(text_seq_len + grid_height * grid_width, 3)` rotary coordinates for the combined
    sequence: text tokens sit at the origin, image tokens carry their `(0, h, w)` latent-grid
    coordinates."""
...
def pack_latents(latents: torch.Tensor, patch_size: int = 2) -> torch.Tensor:
    """(B, C, H, W) latents -> (B, (H/p)*(W/p), C*p*p) packed patch sequence."""
...
def unpack_latents(latents: torch.Tensor, grid_height: int, grid_width: int, patch_size: int = 2) -> torch.Tensor:
    """(B, S, C*p*p) packed patch sequence -> (B, C, H, W) latents."""
    b, _, channels = latents.shape
    p = patch_size
    c = channels // (p * p)
    latents = latents.view(b, grid_height, grid_width, c, p, p)
    latents = latents.permute(0, 3, 1, 4, 2, 5)
    return latents.reshape(b, c, grid_height * p, grid_width * p)
