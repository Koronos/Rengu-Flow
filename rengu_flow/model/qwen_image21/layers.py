...
Text-to-image only: the joint sequence is ``[text (L) | target image (h*w)]``. The inter-layer
tuple must be tensors-only, so the non-tensor block metadata is re-derived per layer from
shapes instead of shipped: the prefill ``segments`` are the single causal text run
``[(0, L, True)]`` and the modulation mask is ``position >= L`` — both read off a zero-storage
``(L, h, w, 0)`` tensor (host shape metadata, no device sync). The complex RoPE crosses stages
as ``view_as_real`` floats, and an all-valid text mask as a 0-size sentinel.
"""
    ...
    def forward(self, inputs):
        with cuda_autocast():
            ...
            prepared = self.model[0].prepare_inputs(
                ...
                prompt_embeds,
                t.view(-1),
                t2i_img_shapes(h, w, bs),
                build_t2i_img_mask(text_len, h, w, bs, device=noisy_latents.device),
                encoder_mask,
            )
            ...
            layout = prepared.hidden_states.new_empty((text_len, h, w, 0))
...
def _target_token_mask(seq_len: int, text_len: int, device) -> torch.Tensor:
    return torch.arange(seq_len, device=device) >= text_len
    ...
    def forward(self, inputs):
        with cuda_autocast():
            ...
            hidden = self.block(
                ...
                target_token_mask=_target_token_mask(hidden.shape[1], text_len, hidden.device),
                segments=[(0, text_len, True)],
...
class FinalLayer(nn.Module):
    """Adaptive norm + projection over the target tokens only (the loss never sees text)."""
    ...
    def forward(self, inputs):
        with cuda_autocast():
            ...
            text_len, h, w = layout.shape[:3]
            target = hidden[:, text_len:]
