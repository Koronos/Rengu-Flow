...
"""Qwen-Image 2.1 VAE (``AutoencoderKLQwenImage21``), vendored from diffusers `main`
(commit 6256aa7, PR #14804 — ``models/autoencoders/autoencoder_kl_qwenimage21.py``).

Vendored because the class only exists in unreleased diffusers and the pinned release is
0.37.1. diffusers 0.37.1's ``AutoencoderKLQwenImage`` cannot be reused by config: it is the
Wan-2.1-style VAE (3-channel RGB, 8x, 16-dim latents, ``Conv3d`` weights, no residual
down/up blocks), while 2.1 is the Wan-2.2-style residual VAE (RGBA in/out, 16x, 64-dim
latents) specialised to single images, whose causal "3D" convolutions are ``Conv2d`` (4-D
weights). The code is unchanged except for the imports (absolute ``diffusers.*``) and the
dropped ``FromOriginalModelMixin`` (single-file loading is not used); module and parameter
names are unchanged, so the official ``vae/`` folder loads verbatim via ``from_pretrained``.
See NOTICE.md / THIRD_PARTY_NOTICES.md.
"""
...
class AutoencoderKLQwenImage21(ModelMixin, AutoencoderMixin, ConfigMixin):
    r"""
