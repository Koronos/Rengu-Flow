...
class GenericOptim(Optimizer):
    ...
    def get_denominator(self, group, state, grad, state_device):
        ...
        if beta2 == 0 or second_moment_type == "none":
            ...
            # Match ema path: checkpoint / load_state_dict may leave row/col buffers on CPU while grad is CUDA.
            exp_avg_sq_row = state['exp_avg_sq_row'].to(grad.device, non_blocking=True)
            exp_avg_sq_col = state['exp_avg_sq_col'].to(grad.device, non_blocking=True)
            grad_sq = grad**2
            ...
            state['exp_avg_sq_row'] = exp_avg_sq_row.to(state_device)
            state['exp_avg_sq_col'] = exp_avg_sq_col.to(state_device)
